    ...
    () =>
      guests.filter(
        (g) =>
          ...
          g.rsvpStatus === "Waitlist" ||
          g.rsvpStatus === "Pending",
      ),
    [guests],
  );
  const arrivedCount = checkable.filter((g) => g.arrived).length;

  const filtered = checkable.filter((g) => {
      ...
      <div className="max-w-3xl mx-auto px-6 py-8">
        <div className="mb-6">
          ...
          <p className="text-muted-foreground mt-1">
            <span className="font-mono text-lg text-foreground">{arrivedCount}</span> of {checkable.length} expected guests arrived
          </p>
        </div>
        ...
        <div className="flex gap-2 mb-4">
          {(["all", "pending", "arrived"] as const).map((t) => (
            ...
            >
              {t === "all" ? "All" : t === "arrived" ? `Arrived (${arrivedCount})` : `Not yet (${checkable.length - arrivedCount})`}
            </button>
          ))}
        </div>
          ...
          {filtered.map((g) => (
            <div key={g.id} className="flex items-center gap-2">
              <button
                ...
                className={`flex-1 text-left p-4 rounded-xl border transition flex items-center gap-4 ${
                  g.arrived
              ...
              >
                ...
                <div className="text-right">
                  ...
                  {(g.rsvpStatus === "Pending" || g.rsvpStatus === "Waitlist") && (
                    <div className="text-[10px] uppercase font-mono text-muted-foreground">{g.rsvpStatus}</div>
                  )}
                </div>
              </button>
              {!g.arrived && (
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    updateGuest(g.id, { rsvpStatus: "No-show" });
                    toast(`${g.name} marked as no-show`);
                  }}
                  className="shrink-0 text-xs px-2 h-8 rounded-md border border-input hover:bg-destructive/10 hover:text-destructive text-muted-foreground"
